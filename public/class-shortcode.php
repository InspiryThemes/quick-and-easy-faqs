...
class Shortcodes {
	...
	public function display_faqs_list( $attributes ) {
		...
		} elseif ( $style === 'filterable-toggle' ) {
		...
		if ( $faqs_query->have_posts() ) :
			while ( $faqs_query->have_posts() ) :
                ...
                <div id="qe-faq-<?php the_ID(); ?>" class="qe-faq-content">
                    <h4 class="qe-faq-question-title"><i class="fa fa-question-circle"></i> <?php the_title(); ?>
                    </h4>
					<?php the_content(); ?>
                    <a class="qe-faq-top" href="#qe-faqs-index"><i
                                class="fa fa-angle-up"></i> <?php _e( 'Back to Index', 'quick-and-easy-faqs' ); ?>
                    </a>
                </div>
			<?php
			endwhile;
			...
			foreach ( $faqs_queries_array as $faqs_query ) {
				...
				if ( $faqs_query->have_posts() ) :
					while ( $faqs_query->have_posts() ) :
						...
						?>
                        <div id="qe-faq-<?php the_ID(); ?>" class="qe-faq-content">
                            <h4 class="qe-faq-question-title"><i
                                        class="fa fa-question-circle"></i> <?php the_title(); ?></h4>
							<?php the_content(); ?>
                            <a class="qe-faq-top" href="#qe-faqs-index"><i
                                        class="fa fa-angle-up"></i> <?php _e( 'Back to Index', 'quick-and-easy-faqs' ); ?>
                            </a>
	...
	private function filterable_toggles_faqs( $filter_array = null ) {
        ...
        <ul class="qe-faqs-filters-container">
            <li class="active"><a class="qe-faqs-filter" href="#*"
                                  data-filter="*"><?php _e( 'All', 'quick-and-easy-faqs' ) ?></a></li><?php
			$faq_groups = get_terms( 'faq-group' );
			if ( ! empty( $faq_groups ) && ! is_wp_error( $faq_groups ) ) {
				foreach ( $faq_groups as $faq_group ) {
					if ( empty( $filter_array ) ) {
